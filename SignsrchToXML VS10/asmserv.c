// note by Luigi Auriemma: I have removed/modified the printfloat functions
...
int Decodeaddress(ulong addr,uchar *symb,int nsymb,uchar *comment) {
    if(symb) _snprintf(symb, nsymb, "%08lX", addr);
...
int Printfloat4(uchar *s,float f) {
  int k;
  ulong *l;
  l = (ulong *)&f;
  if (*l==0x7F800000L)
    k=sprintf(s,"+INF 7F800000");
  else if (*l==0xFF800000L)
    k=sprintf(s,"-INF FF800000");
  else if ((*l & 0xFF800000L)==0x7F800000L)
    k=sprintf(s,"+NAN %08lX",*l);
  else if ((*l & 0xFF800000L)==0xFF800000L)
    k=sprintf(s,"-NAN %08lX",*l);
...
int Printfloat8(uchar *s,double d) {
  int k;
  // too much warnings and errors, removed
    k=sprintf(s,"%#.16lg",d);
  return k;
};
...
int Printfloat10(uchar *s,long double ext) {
  int k;
  // too much warnings and errors, removed
  k=sprintf(s,"%#.16lg",(double)ext);
  return k;
};
...
int Print3dnow(uchar *s,uchar *f) {
  int n;
